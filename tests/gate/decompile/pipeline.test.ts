...
test("the pass registry lists the M5 passes in dependency order", () => {
  ...
  assert.deepEqual(REGISTRY.map((p) => p.name), ["loop-cond", "for-header", "switch-raise", "if-chain", "label-clean", "expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "fn-naming", "var-naming", "jsx-recover"]);
  assert.deepEqual(enabledPasses({ stage: "A" }).map((p) => p.name), ["loop-cond", "for-header", "switch-raise", "if-chain", "label-clean"]);
  assert.deepEqual(enabledPasses({ skip: ["loop-cond"] }).map((p) => p.name), ["for-header", "switch-raise", "if-chain", "label-clean", "expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "fn-naming", "var-naming"]);
