...
test("the pass registry lists the M5 passes in dependency order", () => {
  ...
  assert.deepEqual(REGISTRY.map((p) => p.name), ["loop-cond", "for-header", "label-clean", "expr-rebuild", "global-access", "call-shape", "fn-naming", "var-naming"]);
  assert.deepEqual(enabledPasses({ stage: "A" }).map((p) => p.name), ["loop-cond", "for-header", "label-clean"]);
  assert.deepEqual(enabledPasses({ skip: ["loop-cond"] }).map((p) => p.name), ["for-header", "label-clean", "expr-rebuild", "global-access", "call-shape", "fn-naming", "var-naming"]);
