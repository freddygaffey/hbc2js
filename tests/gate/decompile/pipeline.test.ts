...
test("the pass registry lists the M5 passes in dependency order", () => {
  ...
  // below.
  assert.deepEqual(REGISTRY.map((p) => p.name), ["finally-dedup", "loop-cond", "for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean", "expr-rebuild", "global-access", "globalthis-dead-store", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "yield-recovery", "yield-loop", "async-recovery", "arguments-form", "literal-forms", "try-clean", "class-recover", "super-call", "ctor-this", "private-fields", "jsx-recover", "fn-naming", "reg-split", "var-naming"]);
  assert.deepEqual(enabledPasses({ stage: "A" }).map((p) => p.name), ["finally-dedup", "loop-cond", "for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean"]);
  assert.deepEqual(enabledPasses({ skip: ["loop-cond"] }).map((p) => p.name), ["finally-dedup", "for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean", "expr-rebuild", "global-access", "globalthis-dead-store", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "yield-recovery", "yield-loop", "async-recovery", "arguments-form", "literal-forms", "try-clean", "class-recover", "super-call", "ctor-this", "private-fields", "fn-naming", "reg-split", "var-naming"]);
