...
test("the pass registry lists the M5 passes in dependency order", () => {
  ...
  assert.deepEqual(REGISTRY.map((p) => p.name), ["loop-cond", "for-header", "expr-rebuild", "global-access", "call-shape", "fn-naming"]);
  assert.deepEqual(enabledPasses({ stage: "A" }).map((p) => p.name), ["loop-cond", "for-header"]);
  assert.deepEqual(enabledPasses({ skip: ["loop-cond"] }).map((p) => p.name), ["for-header", "expr-rebuild", "global-access", "call-shape", "fn-naming"]);
