...
test("the pass registry lists the M5 passes in dependency order", () => {
  ...
  // below.
  assert.deepEqual(REGISTRY.map((p) => p.name), ["loop-cond", "for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean", "expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "try-clean", "jsx-recover", "fn-naming", "reg-split", "var-naming"]);
  ...
  assert.deepEqual(enabledPasses({ skip: ["loop-cond"] }).map((p) => p.name), ["for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean", "expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "try-clean", "fn-naming", "reg-split", "var-naming"]);
