...
// P-19 in docs/PUSHBACK.md). Still loaded through a *non-literal* dynamic
...
for (const version of ["v94", "v99"]) {
  ...
  test(`try-shape keeps the guard whose over-reaching block can throw at ${version}`, () => {
    ...
    // stays (spec 22 §4.1; PUSHBACK P-19 corrects this test's fixture: the
  ...
  test(`try-shape drops both provably-redundant guards in 12-try-catch-finally-return at ${version} (PUSHBACK P-19)`, () => {
  ...
  test(`try-shape changes no structural control flow at ${version} (annotation-only)`, () => {
    ...
    // entire purpose, demonstrated by 12 and 15 above (PUSHBACK P-19).
