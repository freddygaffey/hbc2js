...
test("PL-07: registry order must satisfy after/before; skip/only/stage select", () => {
  ...
  assert.deepEqual(enabledPasses({ skip: ["for-header"] }).map((p) => p.name), ["loop-cond", "switch-raise", "if-chain", "label-clean", "expr-rebuild", "global-access", "call-shape", "template-literal", "fn-naming", "var-naming"]);
  assert.deepEqual(enabledPasses({ stage: "B" }).map((p) => p.name), ["expr-rebuild", "global-access", "call-shape", "template-literal", "fn-naming", "var-naming"]);
...
test("PL-05: --passes=none reproduces the M4 emitter output byte for byte", () => {
  ...
  assert.equal(decompile(bytes, { passes: { skip: ["loop-cond", "for-header", "switch-raise", "if-chain", "label-clean", "expr-rebuild", "global-access", "call-shape", "template-literal", "fn-naming", "var-naming"] }, moduleName: "x" }).code, none);
});
