...
test("PL-07: registry order must satisfy after/before; skip/only/stage select", () => {
  ...
  assert.deepEqual(enabledPasses({ stage: "B" }).map((p) => p.name), ["expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "yield-recovery", "async-recovery", "arguments-form", "literal-forms", "try-clean", "class-recover", "fn-naming", "reg-split", "var-naming"]);
...
test("PL-05: --passes=none reproduces the M4 emitter output byte for byte", () => {
  ...
  assert.equal(decompile(bytes, { passes: { skip: ["loop-cond", "for-header", "for-in", "for-of", "switch-raise", "if-chain", "try-shape", "label-clean", "expr-rebuild", "global-access", "call-shape", "default-params", "destructure", "spread-rest", "template-literal", "optional-chain", "object-literal", "yield-recovery", "async-recovery", "arguments-form", "literal-forms", "try-clean", "class-recover", "fn-naming", "reg-split", "var-naming"] }, moduleName: "x" }).code, none);
});
