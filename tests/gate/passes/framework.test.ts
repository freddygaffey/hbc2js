...
test("PL-07: registry order must satisfy after/before; skip/only/stage select", () => {
  ...
  assert.deepEqual(enabledPasses({ skip: ["for-header"] }).map((p) => p.name), ["loop-cond", "label-clean", "expr-rebuild", "global-access", "call-shape", "fn-naming", "var-naming"]);
...
test("PL-05: --passes=none reproduces the M4 emitter output byte for byte", () => {
  ...
  assert.equal(decompile(bytes, { passes: { skip: ["loop-cond", "for-header", "label-clean", "expr-rebuild", "global-access", "call-shape", "fn-naming", "var-naming"] }, moduleName: "x" }).code, none);
});
