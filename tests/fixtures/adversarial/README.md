...
| # | Name | Pattern | Status | Notes |
|---|---|---|---|---|
| 28 | this-binding-extracted | Method extracted and called — this binding | PASS-vs-VM | Decompiled matches the Hermes VM *and* Node's real sloppy/CJS execution (`extracted: undefined-value`) at v94/v96/v99 — standard spec-conformant sloppy `this` substitution, not a Hermes divergence. Committed expected.txt is wrong (ESM-forced strict mode from this repo's own `package.json`, see docs/BUGS.md) |
| 29 | var-hoisting-redeclaration | Var hoisting with same-name function | PASS-vs-VM | Decompiled matches the Hermes VM and Node's real CJS execution at v94/v96/v99 — no actual redeclaration conflict under sloppy/CJS semantics. Committed expected.txt is a spurious ESM `SyntaxError` artifact, same root cause as 28 (see docs/BUGS.md) |
