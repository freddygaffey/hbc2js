...
function applyOverlay(fnIndex: number, body: readonly Stmt[], store: OverlayStore): { readonly body: readonly Stmt[]; readonly collisions: CollisionFlag[] } {
  const names = store.activeNamesForFn(fnIndex);
  if (names.size === 0) return { body, collisions: [] };
  ...
  if (mapping.size === 0) return { body, collisions };
  return { body: renameRegistersInFrame(body, mapping), collisions };
}
