...
| `arguments-form` | `R10` (row 16 is the lowering provenance; `✅ single-version` fails PL-06) | 42, 49 | all | `__hbc_arguments` reads where no param slot aliases → `arguments` | batch 4, spec `docs/specs/passes/23-arguments-form-literal-forms.md` |
| `literal-forms` | 29 (regex), 30 (`TypeOfIs` mask) — both added by spec 23 | 45, 47, 55 (46 is a no-op, see the spec) | all | `new RegExp("…","g")` from a regex-table literal → `/…/g`; `TypeOfIs` mask expansion → `typeof x !== "…"` / `typeof x === "object"`; **BigInt is already `123n` at M4** (PUSHBACK P-18) | batch 4, spec `docs/specs/passes/23-arguments-form-literal-forms.md` |
