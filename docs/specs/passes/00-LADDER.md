...
| `optional-chain` | 25 (single-version) | 48 | all | `x == null ? undefined : x.y` / `getMethod` guard → `x?.y`, `x?.()`; `x == null ? d : x` → `x ?? d` | batch 3, `after: [call-shape]` |
...
| sugar rungs (`template-literal`, `default-params`, `destructure`, `spread-rest`, `optional-chain`, `literal-forms`) | the `Expr` sub-tree of one statement, or a prologue statement run | statements outside the captured run |
