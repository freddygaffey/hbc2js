...
| `async-recovery` | 19 | 27, 28 | all | `spawnAsync(body)` around a recovered generator → `async function` + `await` | batch 4, `after: [yield-recovery, gen-lowered]`; spec [`25-yield-async-recovery.md`](25-yield-async-recovery.md) — **stage B** (P-24); refuses at ≥97 with R-A4 until `gen-lowered` lands. **Landed 2026-09-05**, registered `after: [expr-rebuild, global-access, call-shape, yield-recovery]` only -- `gen-lowered` cannot be named until it exists; recovers `27` and both of `28`'s async functions at 84/94/96 |
...
| `arguments-form` | `R10` (row 16 is the lowering provenance; `✅ single-version` fails PL-06) | 42, 49 | all | `__hbc_arguments` reads where no param slot aliases → `arguments` | **merged** (2026-09-05, `src/passes/arguments-form/`), spec `docs/specs/passes/23-arguments-form-literal-forms.md` |
| `literal-forms` | 29 (regex), 30 (`TypeOfIs` mask) — both added by spec 23 | 45, 47, 55 (46 is a no-op, see the spec) | all | `new RegExp("…","g")` from a regex-table literal → `/…/g`; `TypeOfIs` mask expansion → `typeof x !== "…"` / `typeof x === "object"`; **BigInt is already `123n` at M4** (PUSHBACK P-18) | **merged** (2026-09-05, `src/passes/literal-forms/`), spec `docs/specs/passes/23-arguments-form-literal-forms.md` |
...
| `class-recover` | 20 (✅ verified, v98+v99 — upgraded by spec 24) | 32–36 | **98 and 99** (layout E; ≤96 `hermesc` has no class lowering at all) | `CreateBaseClass/DerivedClass` + `Constructor<>`/`NCFunction<>` + `<instance_members_initializer>` → `class` | batch 4, spec `docs/specs/passes/24-class-recover.md`; `after: [expr-rebuild, global-access, call-shape, object-literal]`, `before: [fn-naming, reg-split, var-naming]` (D23 — PUSHBACK P-21 against this row's old `after: [fn-naming]`, **ruled for D23 and corrected here 2026-09-05**); checker class: **class-shape** (§4.3, P-23 ruled), not expression-only. Landed 2026-09-05 for sub-form C2 (the derived form); C1/C3/C4 refuse on F24-5 (PUSHBACK P-38) |
| `super-call` | R14 (readability row -- it recognises the emitter's own lowering of `CreateThisForSuper`/`CallWithNewTarget`/`SelectObject`/`ThrowIfThisInitialized` inside a constructor `class-recover` has already raised) | 33 | **98 and 99**, same gate as `class-recover` | a derived constructor's `rC = Reflect.construct(Object.getPrototypeOf(<the class's own env slot>), [args], new.target)` + the dead `"super() called twice"` guard -> `super(args);`, with the stand-in register becoming the literal `this` | **landed 2026-09-05**, spec `docs/specs/passes/28-super-call.md`, `src/passes/super-call/`; `after: [class-recover]`, `before: [ctor-this, private-fields, fn-naming, reg-split, var-naming]`. Sound because ES2024 13.3.7.1 defines SuperCall as exactly `Construct(activeFunction.[[GetPrototypeOf]](), args, newTarget)` followed by binding the result to `this`. R-SC1 (the superclass must be the class's own binding) is proved from the ENCLOSING body's `_eD_S = <class>` store, written exactly once anywhere -- the one fact a constructor-local matcher cannot see, and the reason this rung reads the whole function body rather than one class node. `ctor-this` needs no change: its R-CT1 keys on `superClass !== null`, still true afterwards, and a derived constructor never contains the `new.target.prototype` allocation R12 folds. Checker: independent re-derivation, an **unchanged** class-definition effect sequence, no new free name, `parses` (which is also what proves the `super(...)` landed in a derived constructor). This is R-CT1's answer: 304 of react-navigation's 448 recovered constructors are this shape. |
| `ctor-this` | R12 (readability row -- it recognises the emitter's own `NewObjectWithParent` lowering inside a constructor `class-recover` has already raised, not a Hermes idiom of its own) | 34, 35, 36 (32 refuses R-CT2, 33 refuses R-CT1) | **98 and 99**, same gate as `class-recover` | a base-class constructor's `rN = new.target.prototype; rM = Object.create(rN === null ? null : typeof rN === "object" ? rN : Object.prototype); ... return rM;` stand-in receiver -> the literal `this` | **landed 2026-09-05**, spec `docs/specs/passes/26-ctor-this.md`, `src/passes/ctor-this/`; `after: [class-recover]`, `before: [private-fields, fn-naming, reg-split, var-naming]`. Sound because a BASE class's `[[Construct]]` binds `this` to exactly `OrdinaryCreateFromConstructor(new.target)` before the body runs, so the allocated register and `this` are the same object on every path; a derived class is refused outright (R-CT1), its `this` comes from `super()`. Checker: independent re-derivation, an **unchanged** class-definition effect sequence (everything it touches is inside a method body, which `effectSequence` does not evaluate), no new free name, `parses`. This is the rung that finally lets `private-fields` fire on a real fixture. |
| `private-fields` | 20 (same row as `class-recover` -- a different opcode family within it, see LOWERING-CATALOGUE.md) | 35 | **98 and 99**, same gate as `class-recover` | `CreatePrivateName`/`AddOwnPrivateBySym`/`Get`/`PutOwnPrivateBySym`/`PrivateIsIn` (lowered by `src/emit/lower.ts`'s "private names" block to `Symbol(...)` + computed-member/`Object.defineProperty`/`hasOwnProperty.call`) → real `#name` field declarations, `obj.#name` reads/writes, `#name in obj` | **merged 2026-09-05; fires for real on fixture 35 since `ctor-this` landed the same day** (docs/BUGS.md 2026-09-01 "class private fields"), `src/passes/private-fields/`; `after: [class-recover, ctor-this]`, `before: [fn-naming, reg-split, var-naming]` -- same ordering reason as `class-recover` (P-21/D23: it also follows register identity through the install's key argument). One private name at a time: refuses (leaves the `Symbol`/computed-member shape untouched) whenever any reference to that name is not one of the four recognised shapes, independently per name. Checker: independent re-derivation (`foldAll(before)` recomputed and compared to `after`), plus no new free name and `parses(after)` -- the same two of class-recover's four obligations that still apply once `effectSequence` is unnecessary (a private-name declare-and-initialise *is* a field declaration's own semantics, not a third form to diff against). **`isThisArg`**: an install is only ever recognised when its own target resolves to literal `this` -- native private fields can only be added to the object a class's own `[[Construct]]` actually brands, and every constructor this codebase decompiled *before `ctor-this`* instead built and returned a separate `Object.create(new.target.prototype)` stand-in, never touching `this` (T2 equivalence, tests/gate/decompile/equivalence.test.ts, caught the unguarded version writing `#balance` onto that stand-in and throwing at construction). The guard is unchanged; what changed is its input. `ctor-this` (R12) proves the stand-in IS the receiver in a base class and substitutes `this`, so fixture 35 now folds `#balance`/`#history` into real field declarations and `this.#balance` accesses, proven end to end by T2 at v99. The private *method* `#record` and the `PrivateIsIn` brand check still do not fold (no recognised shape for either), and are left as the `Symbol`-keyed lowering. |
...
| `iife-reconstruct` | R13 | 75, bundles | 98, 99 (84/94/96 do not inline these IIFEs) | a function owning several SIBLING environments (`hermesc -O` inlined an IIFE but kept the callee's environment) -> each environment's contiguous statement range wrapped back as `(function () { ... })();` | **landed 2026-09-05**, and **not a rung**: it is an emit-side placement step, `src/emit/iife-reconstruct.ts`, spec [`27-iife-reconstruct.md`](27-iife-reconstruct.md) SS3 (it needs the env graph's parent relation and `closureCreationSites`, neither of which reaches `src/passes`). Listed here so the ladder stays the single inventory. |
...
**Count: 31 rungs** (12 stage A, 19 stage B); 27 merged (jsx-recover opt-in) as of the 2026-09-05 ladder landing; remaining: gen-lowered and the hard/unscheduled rows below.
...
gen-lowered ────┼─► finally-dedup ─► loop-cond ─► for-header      │
                ...
                │        │             ├─► switch-raise           ├─► class-recover ─► fn-naming   │
...
| ~~`yield-recovery`~~, `gen-lowered` | `switch` with `generator-state`/`dispatch` scrutinee, `setState`, `return` | `try` regions other than the generator's own — **`yield-recovery` is stage B (P-24) and this stage-A ownership row does not apply to it**; its stage-B ownership is one *generator group* per enclosing body (spec 25 §3.2). `gen-lowered`'s own spec re-reads this row. |
