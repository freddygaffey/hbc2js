...
| `for-of` | 10 (✅ verified, v94+v99 — re-read done 2026-09-05, spec 21; v99 `Mov`-refreshes the `IteratorNext` source and the normal-close state register) | 06, 07 | all | `IteratorBegin` + `IteratorNext` loop + two `IteratorClose` sites → `for (v of it)` | **registered, partial** (2026-09-05, agent/forin): the stage-B traversal blocker is fixed (see `for-in` above) and the rung correctly recovers `06-for-of-array`'s two `break`-free loops and all three of `07-for-of-iterable`'s at some versions — verified non-divergent against the real Hermes VM — but `06`'s `break`-containing loop uses a nested-try/merge-point handler shape the matcher does not yet recognise, and `07` under-recovers at some versions (root cause open). `tests/gate/passes/for-of.test.ts` stays `skip`ped; `docs/BUGS.md`'s `for-of` row (2026-09-05, second one) tracks the gap. |
