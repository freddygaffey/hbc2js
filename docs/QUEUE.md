...
## Now
1. **B — navigator detection precision (product)**: after route resolution, some bare `create<X>Navigator` factory RE-EXPORTS are still mis-counted as navigators (detection imprecision, not resolution — noted in the route-resolution BUGS row). Tighten detectNavigator so a factory re-export with no route registry of its own is NOT counted; and name the real navigators from their now-resolved route sets (176 screens resolve → more navigators should get LicenceNavigator/PayFines-style names). HARD BAR: react-navigation-example 4/54, 6/58 (assert.equal). NSW: report navigators + route-named before→after.
