...
- **FIX spec-27 seam join overfit — 0/9 on real bundles (BUGS.md 2026-09-05 rows; validation: /Users/fred/nsw-hunt/NATIVE-INGEST-TEST.md).** The JS↔native linkage — the marquee/unique-edge feature — links NOTHING on a real Metro bundle: `src/native/seams.ts` `anchorFns` matches `NativeModules`/`TurboModuleRegistry`/`requireNativeComponent` only as JS GLOBALS (fixture-shaped); real bundles bind them as `require()`-locals. FIX: resolve require-bound locals via `index/string-uses.jsonl` / points-to (evidence already present, e.g. fn:8871), not just globals. METHODOLOGY (hard rule): rebuild `tests/fixtures/constructs/66-native-module-seams` Metro-shaped (require-bound) so it FAILS before / PASSES after; ship the fixture+fix+docs in one commit; validate ≥8/9 first-party modules link on the real NSW APK. SECOND (smaller) bug in the same rows: `deps --out` silently no-ops native ingestion on a stale `dist/` (exit 0) — add a loud guard + regression test. Priority: correctness bug in a LANDED feature — orchestrator schedule when appropriate (ahead of net-new native L-levels; not blocked on the Stage-3 UI). Lean agent, model per orchestrator.
...
**Reconciled 2026-09-04**, refreshed **2026-09-05 evening** against `git log --oneline -40`, `docs/AGENT-LOG.md`, `docs/BUGS.md`, `docs/PUSHBACK.md`. Landed items are collapsed to one line with their sha. Items are tagged **LANDED / IN FLIGHT / OPEN / NEEDS FRED**.
...
- **AST pattern "template literal containing a quoted string containing `${…}`"** — LANDED (`c0e4872`, `query: add template-injections — bundle-wide WebView-injection anti-pattern scan (spec 17 §14.3, backlog #9, hunt lead C1)`). Was QUEUE "Now" #9.
- **Ladder specs 23/24/25 — design + acceptance tests, no implementation.** Spec 23 arguments-form/literal-forms (`f066cf1`); spec 24 class-recover (`f0cc041`); spec 25 yield-recovery/async-recovery (`a259788`, merged `45ebaf4`). Implementation candidates queued above (Now #1, #2); spec 23 already has an in-flight impl branch (`agent/spec23-impl`, IN FLIGHT below).
- **SPEC 27 — native-side capability, L1–L9 COMPLETE.** APK ingestion + JS↔native linkage: L7 (`fdbdd9a`), L8 `src/native/reconstruct.ts` (`a61d2e0`), L9 one-command deps walkthrough / APK input in `deps` (`a5038d5`). Real-APK ingestion still needs an NSW `.apk` (NEEDS FRED).
- **projdb `ix_calls_resolved` migration** — LANDED (`8a3747c`, MIGRATION 5). DB-backed artifacts now serve points-to edges.
...
## NEEDS FRED (newest first)

1. **Golden batch — fixture 67 disasm/parse goldens (additive) + the pipeline-speed hash for the ladder chain.** Regeneration needs Fred's approval, reviewed as a batch (CLAUDE.md testing rules). Confirmed: stage-B specs (23) do NOT move the hash; it moves with the yield-recovery (spec 25) implementation. Proposed standing rule (needs ratification): regen is allowed for a ladder rung once that rung's own tests are green and the output diff is attached to the landing report.
2. **NSW `.apk` needed for real-APK native ingestion** (spec 27) — none available on Mac or deb; blocks the real-APK L-steps beyond the synthetic fixtures.
3. **PUSHBACK triage P-18..P-26** (`docs/PUSHBACK.md`): P-18 — strike the BigInt-ladder row (no-op, already delivered by M4's emitter); P-21 COLLISION — the id is used by both spec 24 (landed on main) and `agent/spec23-impl` (in flight) — renumber the latter to P-27 at landing; P-24 — spec 25's rungs are stage B not A, `docs/specs/passes/00-LADDER.md` §1.1/§2/§3.1 still need editing to match; P-25 — `docs/lowering/async-await.md` §3/§6 (T13) is a stale v99 reading, needs correcting; P-26 — `00-LADDER.md` §4.3's check taxonomy needs a decision for yield/gen (neither CF-preserving nor expression-only).
4. **Spec 24 (class-recover) §6.5 C5** wants its own spec + an `-O0` fixture; §6.7's second reader is row 20 — confirm both before class-recover implementation starts.
5. **Spec 23 (arguments-form/literal-forms):** catalogue row 16's aliasing claim conflicts with `src/runtime/helpers.ts` §8 — needs reconciling; the identity-regression construct fixture is designed but not shipped (needs goldens).
6. **Spec 27 §5 decisions:** DEX vs baksmali, the first-party heuristic, the third-party seed list, seam-UI ordering; the aapt/baksmali cross-check needs an Android SDK installed on deb.
7. **Two spec 27 naming/ownership items:** L5's `GET /api/native/impl/:fn` route collides in name with the pre-existing `query native [--fn N]` verb (disambiguated in the spec text, `docs/specs/27-native-side.md` line ~477, but not yet renamed in code); L3's BUGS row (host-member receiver) needs an owner decision.
8. **Golden/snapshot regeneration batch (general).** The orchestrator queues regenerations; regen needs Fred's approval, reviewed as a batch (CLAUDE.md testing rules). Nothing regenerated inside an impl task.
9. **`MIN_HOSTED` sweep ratchet** dropped 100 → 10 by design (`docs/AGENT-LOG.md` 2026-09-05: placement now has almost nothing to place). Confirm the new floor is the intended standing bar (with new `MIN_DUPLICATED`/`MAX_STILL_AMBIGUOUS`).
10. **The 18 unaligned closure chains** (`docs/reports/2026-09-05-ambiguous-closure-env.md` leftover 4, 19 of the 26 residual unbound names). Decide: chain alignment by *owner function* rather than position, OR leave them as `W_AMBIGUOUS_CLOSURE_ENV` forever, recorded in `docs/DECISIONS.md`. Fred, not an implementer.
11. **`namesAgreeAcrossSites` trade** (leftover 7, the 3 `_fn<n>` children that stay behind). Either one instance per creation context (per-instance `parentOf`) or a creation-based `lexicalSubtree` so the join never fires for them and they duplicate instead — a `src/cfg/**`/whole-emitter change needing its own spec. Fred's call on which.
12. **Graph-view library: dagre vs elkjs — DECIDED 2026-09-05 (delegated to the orchestrator by Fred): React Flow (`@xyflow/react`, MIT) + `@dagrejs/dagre` (MIT); elkjs rejected (EPL-2.0). Recorded as `docs/DECISIONS.md` D28, implemented per `docs/specs/25-ui-graph-view.md`.** Still reserved for Fred: the rest of the UI stack in that report (art-direction seed, token format, reference set/match strictness, lint hard-fail vs warn).
13. **v84 members of the arity/arguments-aliasing and counter-inc-dec-reset rows** (`docs/BUGS.md` 2026-09-05, both `open`): v94/v99 reclassified INCONCLUSIVE, v96 D14-legit; only the unsampled v84 members keep both rows open. Decide whether to obtain/point a v84 VM to close them or accept them as INCONCLUSIVE.
14. **Fixture 47 (`tests/fixtures/adversarial/47-spread-non-iterable-message`) registration.** The fix (`228c53c`, `9a415f2`) placed it in `adversarial/` because a `constructs/` home would need a `KNOWN_DIVERGENT_FIXTURES` entry in `src/harness/reference-policy.ts` — tried and reverted (its ratchet test is out of scope for a shared-tree task). Decide whether to register it properly.
15. **INCONCLUSIVE rate.** With INCONCLUSIVE never counted as PASS (D-rule) and several campaign-2 families now INCONCLUSIVE, decide the target/threshold for the scoreboard's INCONCLUSIVE column and whether it gates.
16. **`--split` per-module isolation failure-mode.** `acff415` resolves object shapes across blocks for `--split`; confirm the standing policy for a module that still fails isolation under `--split` (stub-and-continue vs fail-closed), since `--split` is the product path.
17. **Nested-closure disasm-follow default.** `894a8e2` makes the disasm pane follow the cursor into a nested closure (spec 05 §16.2). Confirm this is the default behaviour (vs opt-in) for the shipped UI.
18. **Static-proof finding confirmation** (carried from spec 17 / spec 11 §4.1): `open→confirmed` currently requires a DYNAMIC evidence ref, so a statically-provable finding (parsed-but-never-checked signature, hardcoded key) can't reach `confirmed`. Decide whether to allow confirmation via a fidelity-checked STATIC proof — touches spec 11's finding-status model.
19. **Delete-or-keep deprecated `tools/equiv/`** (128K, provably unimported since M3 `baf9972`; 2026-09-03 sweep finding 5). Deletion requires rewording `docs/EQUIVALENCE.md`'s reproducibility claim.
20. **UI art-direction seed** (spec 26 §4.1, spec 20 §1.5) — the one input the whole aesthetics playbook rests on. Any ONE of: edit `ui/themes/dark.json`'s ~20 values, name a theme ("Darcula-like"), or drop 2–3 reference screenshots into `docs/ui-refs/`. Blocks landing 26.3's values (not its structure).
21. **Visual-baseline approval** (spec 26 §4.2) — landing 26.7 wants the FIRST commit of Playwright screenshot baselines; golden artifacts, so Fred approves the batch (CLAUDE.md testing rules).
22. **`recompile_edit` from the UI** (spec 26 §4.3, spec 17 §13, spec 21 §5.2) — may the UI drive the one operation that produces a modified binary, and is its sandbox a git worktree or a temp copy? Blocks landing 26.8.
23. **First-run information hierarchy** (spec 26 §4.4, spec 20 §1.6) — which panes are open on opening a project, what is one click vs three. One sentence or one screenshot; landing 26.10 implements whatever it says.

Struck (DONE, was item 6): refresh-the-deb-clone-before-campaign-2-compute — campaign 3 already runs from a fresh clone `~/hbc2js-c3` at `1e1fe39` (see IN FLIGHT above); no longer a decision Fred needs to make.
