...
- **B — Segregation MILESTONE 3 (screens/navigators, spec 08 §3)**: detect navigators (createXNavigator via deps→@react-navigation), walk route configs for {name, component} pairs → src/screens/<Name>Screen.js, stores/slices. Needs a router-heavy fixture: react-navigation-example-0.85.3 (fetch via its fetch.sh; if network-blocked in-agent, run the fetch on deb or note it). Metric: % src named + screens/navigators detected on that fixture. This is where the naming % actually moves.
