...
8. **native `bridge-module` surface** (BUGS 2026-09-02 row) — *Sonnet*. `src/artifact/native.ts` §2.5: builtin/host-global rows exist, `bridge-module` rows were scope-cut. Why: feeds the NativeModules JS↔native linkage (hunt gap #1).
...
5. **Graph-view library: dagre vs elkjs.** `docs/reports/2026-09-04-ui-aesthetics-libs.md` "RESERVED FOR OWNER": elkjs is EPL-2.0 → recommends dagre (MIT) with React Flow. Ratify the graph lib (and the rest of the reserved UI stack: art-direction seed, token format, reference set/match strictness, lint hard-fail vs warn).
