...
5. **`deb` as the compute/CI server (Fred, 2026-09-01)** — Sonnet. (a) `tools/deb/run.sh <cmd>`: rsync the current worktree (excluding node_modules/.git) to `deb:~/hbc2js-ci/<branch>/`, `npm ci` there if the lockfile changed, run the command (`npm test`, `npm run test:all`, tier-1 `tools/e2e/roundtrip-corpus.ts`, bulk jobs) under nohup with a job id, stream the tail back, exit with its status — so agents and the orchestrator run heavy tests on deb's 32 cores instead of this Mac. (b) A tiny job queue on deb (`~/hbc2js-ci/queue/` + a runner loop) so several agents can submit without collisions; `run.sh --status`/`--log <id>`. (c) Prepare a self-hosted GitHub Actions runner for the `ci` and `sweep` workflows on deb (script + the `runs-on: [self-hosted, linux]` change behind a workflow input; the registration token step is left for Fred). (d) Symlink/copy hermesc + hermes-vm builds and the corpus on deb (allowed). Windows laptop (`win`, 20 cores, no node/git yet): optional follow-up — install node 22 + git and register it as a second runner.
