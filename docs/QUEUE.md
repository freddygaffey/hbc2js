...
- **B — Segregation MILESTONE 2 (naming, spec 08 §2.1)**: name app modules from single-module signals (entry→index.js; registerComponent name→App.js; displayName; default-export identifier; createSlice→store/xSlice.js), no cross-module route walking. Collision suffix (Fred open Q: ordinal vs content-hash — default ordinal, note it). Metric: % src modules with a non-module_N name on rn-template. Then milestone 3 = screens/navigators (react-navigation-example fixture).
