...
M5 pass ladder flow
- Passes are specced in batches of five by the architect (`docs/specs/passes/NN-<name>.md`: match conditions from the catalogue row, rewrite output, check obligations, fixtures, acceptance), implemented one per Sonnet agent against its spec, reviewed briefly by a stronger model before the next pass builds on it. The gate (501/501 with passes on) is the regression bar for every pass.
