...
| 4 | Mutation-test the pass checkers (Stryker over `src/passes/*/check.ts`) | in progress 2026-09-01 | M | Stryker not adopted (too slow for the budget); targeted hand-built mutations instead. `tests/gate/passes/checker-mutation.test.ts`: expr-rebuild, var-naming, fn-naming, global-access, call-shape covered (2-3 mutations each); var-naming/fn-naming/global-access/call-shape all caught their mutation. expr-rebuild's wrong-value hole (`docs/BUGS.md` 2026-09-01 row) is now fixed: `check` re-derives the folded site and calls `rewrite.ts`'s own pure `rewrite()` to recompute the expected `after`, comparing structurally instead of trusting the read/write count delta alone; `checker-mutation.test.ts` now asserts rejection of a wrong constant, a wrong operator, and a wrong operand register (3 active cases, 0 `test.todo` left for expr-rebuild). Remaining: the 5 stage-A (tree-IR) checkers and template-literal/jsx-recover not yet empirically probed (budget) — next agent picks up there. |
