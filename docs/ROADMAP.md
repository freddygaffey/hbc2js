...
Four stages: **1 readable src/**, **2 analysis & tagging environment**, **3 Ghidra-like RE UI** (over the Stage-2 substrate), **4 deobfuscation + dead-code**.
...
| 13 | **Deobfuscation** — string-array-decode (`_0x..(i)`→literal) + obfuscation rungs | 4 | after Stage 3 | deferred |
| 14 | **Dead-code = ANNOTATE, not delete** — tag provably-dead in the store; **surface reachable-but-not-from-UI (hidden admin/debug/flagged routes) as FINDINGS** | 4 | after Stage 3 | deferred |
| 15 | **Ghidra-like RE UI** — graphical front-end over the Stage-2 substrate (function/module navigator, source+disasm views, xref panels, call/CFG graph, rename/comment/tag/findings write UI, search) consuming the SAME project-DB/file API as the MCP. Human-editing UI deferred here per spec 18. | 3 | after Stage 2 | investigation queued |
...
- **Stage 4 (deobfuscation + dead-code) strictly last.** Stage 3 = the Ghidra-like RE UI, built on the Stage-2 substrate (project DB + files + the MCP's read/write API); the UI and MCP share one backend contract. Deobfuscation + dead-code wait until both are standing.
