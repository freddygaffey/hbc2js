...
- `ui/src/api.ts` — a `fetch` client over `VITE_API_BASE` (default
  ...
  | GET | `/api/template-injections?module=&limit=` | `TemplateInjections` — bundle-wide WebView-injection anti-pattern scan (hunt lead C1), spec 17 §14.3. Live: needs the server's `--hbc`; each row is inlined with the containing function's `fnName`/`size`. Ranked by substitutions-inside-quotes desc, then `fn`. Answered off the critical path (`templateInjectionsAsync`) and cached after the first call: the scan decodes every function in the bundle (23 s on a real 12 MB app) and `limit` CANNOT be pushed into it, because the ranking is only known once every row exists. **Contracts only — no UI pane yet.** |
  ...
  | GET | `/api/search/functions?q=&regex=&cursor=&limit=` | `SearchPage<FunctionMatch>` — `limit` (1..50) narrows the page; a name match is cheap, so this one is answered inline |
  | GET | `/api/search/source?q=&regex=&cursor=&limit=` | `SearchPage<SourceMatch>` — a bundle-wide grep. Answered OFF the event loop's critical path (`McpResources.searchSourceAsync`, `src/incremental.ts`): the scan hands the loop back every 8 ms, so every other route keeps answering while it runs, the same guarantee `/api/segregation` gives via its worker. `limit` (1..50) is pushed down INTO the scan — it stops as soon as the page plus one row is filled, and the page then carries `partial: true`, meaning `total` is a lower bound on the match count, not the count |
