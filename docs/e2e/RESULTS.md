...
| `diff:GetById/LoadConstNull`, `diff:GetEnvironment/LoadConstNull` | 170 + 41 | — | 1511 + 975 | `LoadThisNS` printed as the explicit `this` coercion in sloppy functions. FIXED 2026-09-05 (docs/BUGS.md Resolved, "LoadThisNS lowering"): re-measured on rn-template-0.72 v94 passes-on, `--normalise strong`, both buckets are **0** (no `LoadConstNull` bucket of any kind survives; 435/435 modules, 4125 functions, IDENTICAL 1804 = 43.73%). react-navigation and Service NSW were not re-measured, so their counts above are pre-fix. |
