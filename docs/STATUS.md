...
- **D17c bulk signature build on `deb` — NEXT FREE SLOT** (Fred): short agent writes `tools/pkgsig/bulk-build.sh` + package list (top ~3,000 RN-relevant packages: derive from npm download stats filtered to react-native/expo keywords + known RN ecosystem, pin the 3–5 most-downloaded versions each), installs Node 22 via fnm and the hermesc set on deb, starts the run under `nohup` with a progress log and per-package resumability, then exits. A later short agent (1–2 h) checks the log, assembles the DB (separate distribution, fetch-on-demand; not committed to the repo beyond an index), reports coverage against the corpus apps. Loop ticks: only glance at the log, never relaunch the run while it is alive.
