...
- **Deps matcher tier bug (next deps-rotation item, before hint tier)**: `src/deps/match.ts`'s "high" tier can be cleared by one coincidentally-matching module in a package with very few modules (js-md5, @emotion/react on react-navigation-example with the fixed bulk DB). Require a minimum absolute matched-module count and function-coverage floor for high; add a D17d regression (those two must not be confirmed); re-measure on both own fixtures.
...
- **New finding, not yet fixed (`src/deps/match.ts`, out of the above task's ownership)**: `js-md5` and `@emotion/react` (present in the bulk DB but not real dependencies of `react-navigation-example`) still clear `match.ts`'s "high" confidence tier via a single coincidentally-matching module each, because `scorePackage`'s `moduleExactHits >= 1 && moduleCoverage >= 0.05` threshold is lenient for packages with very few total modules (`js-md5` has only 2 non-baseline modules, so 1 hit = 50% coverage) — root cause is generic small-module/barrel-file collision, unrelated to react/react-native/Metro boilerplate, so the D17c baseline-subtraction fix above cannot and does not touch it. `docs/PACKAGE-SIGNATURES.md` §6.1.3 has the full evidence. Next: tighten `match.ts`'s module-coverage/count threshold for small packages (or require corroborating evidence — e.g. a minimum absolute module count, not just a percentage) before re-measuring D17d's gate.
