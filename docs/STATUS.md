...
| 5 readable | `hbc2js segregate` (milestone 3, screens/navigators): detected / named / fuzzy-match vs `.map` | n/a (no navigation) | WITH deps: 50 screens, 3 navigators, mean fuzzy 0.687 (unchanged, generalization-sweep fix below doesn't touch this column). NO deps (2026-09-02, deps-optional detection): 6 navigators unchanged, screens re-pinned 58→52 (generalization-sweep fix, same day: the 6 dropped were themselves the false-positive bug this fix closes, bundled inside this fixture's own deps — see docs/BUGS.md). Navigators now named from their own route set's common prefix when resolvable (`LicenceNavigator.js`), a dominant-domain/role name when the set has no shared prefix (`LicenceNavigator` off a plurality domain, else `RootNavigator`/`MainTabNavigator`, 2026-09-02 4th revisit), else `<Type>Navigator` | NO deps only (`deps` >10 min, never run): 176 screens, 17 navigators — **confirmed unchanged** by the 2026-09-02 generalization-sweep fix (route-registry literal now gated on navigator-connection; NSW's own registries are all navigator-connected already, no false positives to lose). Same fix: local-corpus Brex 71→0, Uniswap 45→0 fake "screens" on apps with no react-navigation usage (`docs/BUGS.md` 2026-09-02 row) | docs/specs/08-segregation.md §3/§6 milestone 3 result + "Eighth revisit", 2026-09-02; `tools/e2e/name-accuracy.mjs [--no-deps]` |
...
`npm test` (this run, 2026-09-02): 1713 tests, 1710 pass, 0 fail, 3 skipped,
...
## Open bugs — 24 open / 29 resolved, docs/BUGS.md (triaged 2026-09-01, QUEUE 4; +2 rows 2026-09-02 destructure landing; +1 row 2026-09-02 spread-rest landing; +1 row 2026-09-02 generalization-sweep screen-naming fix)
