...
| P28 LLM readability layer (spec 28, Fred 2026-09-11) | complete on main 13c6352c: 5 landings + 1b/4b/4c/4d -- backends (claude-cli DEFAULT on the plan, API opt-in, replay, heuristic), naming pass, equiv-gated rewrites, DB transaction log + file ops + revert/trace, 7 MCP tools, UI suggestion pane + 4 actions (queued jobs), evaluation loop + adversarial re-check | gate 3613/0 at 13c6352c; readability tests 97 (94 green, 3 skipped: held-out coverage/quality need a recording, NSW leg needs HBC2JS_NSW_HBC); real plan smoke 2 targets ~5.1k tokens in, ~55 s per call; file-op pass 5/5 files traced, 4/4 reverts exact | docs/specs/28-llm-readability.md section 10; docs/READABILITY.md |
...
`fn-naming`, `reg-split`, `var-naming`. Structure rungs run before the renaming block (D23); `reg-split` default-on.
Spec 25 recovers `function*`/`async` for non-delegating acyclic shapes; the
pipeline-speed pin is `6c2f2dbe` (rn-template diff:
