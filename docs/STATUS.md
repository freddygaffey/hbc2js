...
- **Emitter allowlist for global-access (small, src/emit — needs Opus or Fable since it touches emit semantics, not a pass)**: `src/emit/scope-check.ts` EM-01 `checkBindings` rejects any bare identifier outside a narrow `KNOWN_GLOBALS` set, so `global-access` (R2) can only fold guards on ECMAScript intrinsics, never host globals (`print`, `console`, `alert`, `window`). Widen the check to permit a bare reference the decompiler deliberately emitted for a proven global (thread a marker from the pass, or treat any name the module never binds as a global read per D14), with a regression test that `print`-guarded fixtures fold and still decompile. Lifts R2 from ~61% toward its 95% target.
