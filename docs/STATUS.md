...
- [ ] M5 Pass ladder: one construct fixture per iteration as matcher/writer/checker pass (D12), catalogue row per pass — **11 rungs merged: loop-cond, for-header, switch-raise (S1), if-chain, label-clean, expr-rebuild, global-access, call-shape, template-literal, fn-naming, var-naming; 2 PR-13 bugs queued (global-access, maxDepth)** (`while`, `do…while`, `for`).
