...
- **CI fix #2 (next free slot, short Sonnet)**: CI still red on 21706e5/8204d38 — `build-test` sets `HBC2JS_REQUIRE_ORACLES=1` without installing hermes-dec (7.B diff, hbc-file-parser cross-check fail) and the fuzz T8 test; `oracle-hermes-dec` job lacks `tools/get-hermesc.sh` compilers (identity gate test fails). Either install both oracles in the jobs that require them, or drop REQUIRE_ORACLES where an oracle is intentionally absent. Green CI unblocks PR #9.
...
- **D17c bulk signature build — RUNNING on `deb`** (started 2026-08-30, Claude Sonnet 5): `tools/pkgsig/bulk/packages.json` (2,998 packages, 13,319 package@version selections, ~4.4 versions/pkg spanning 2022-2026) and `tools/pkgsig/bulk/run.sh` are committed. Node 22 (fnm) + `tools/get-hermesc.sh all` installed on `deb`; `npm test` on Linux is 548/550 (2 perf-budget-only failures, not decompiler bugs — see AGENT-LOG). Two Metro scaffolds (RN 0.72.17 -> HBC94/96, RN 0.87.1 -> HBC98/99), 16 cloned slots each, drive `~53,300` (package, version, hbcVersion) build jobs at 16-way parallelism (`xargs -P 16` + a flock-based slot semaphore), each `npm install <pkg>@<version> --ignore-scripts` (never runs package code) -> Metro bundle -> matching `hermesc` -> fingerprint -> write into `~/hbc2js-bulk/db/` (D17c/schema-2 format, resumable — skips anything already on disk). Estimated 4-6 hours total. **Check progress**: `ssh deb '~/hbc2js/tools/pkgsig/bulk/run.sh status'` (also `... failures` for recent failure reasons). Known, expected failure classes (not bugs): packages with no bare-`require(pkg)` entry point (e.g. `@babel/runtime`, meant to be deep-imported) can't be fingerprinted by this bulk driver's generic entry file; fingerprinting `react-native`/`react-dom` themselves against a scaffold already pinned to a different version of the same package sometimes fails to resolve the CLI. A later short agent: let the run finish (or run long enough), assemble/publish the DB (separate distribution, fetch-on-demand; not committed to the repo beyond an index), reports coverage against the corpus apps. Loop ticks: only glance at the log, never relaunch the run while it is alive.
...
- **On-device round-trip (D16 C6)**: build the RN 0.72 template APK (Android SDK at ~/Library/Android/sdk), pull `index.android.bundle` → `hbc2js` → swap the **decompiled .js directly** into the APK as `index.android.bundle` (Hermes compiles plain JS at load; no recompile needed) — optional second variant pre-compiles with `hermesc` → re-sign (debug key) → `adb install` on the connected tablet → launch → screenshot + logcat comparison against the original build. Automate with **Maestro** flows (tap/assert/screenshot YAML) run against original and decompiled installs, comparing assertions, logcat JS output and pixel-diffed screenshots; `tools/device-roundtrip.sh` wraps build → install → flow → swap bundle → re-sign → install → flow → diff. Tablet first; headless AVD on `deb` as the CI fallback. Own/open-source apps only (re-signing breaks integrity-checked proprietary apps). **Bumped to first free slot.** Then repeat with the react-navigation example.
