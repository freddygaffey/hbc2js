...
- **D17c fix (next deps-rotation item, before hint tier)**: `tools/pkgsig/bulk/build-one.mjs` skips the foundation/toolchain-baseline subtraction (PACKAGE-SIGNATURES §5.2), so bulk signatures collide across unrelated packages (thousands of false confirmedDeps on rn-template). Fix build-one.mjs to subtract the per-(RN, hbc) baseline, then re-fingerprint the existing ~/hbc2js-bulk/db on deb *from the cached bundles* (don't re-download), re-assemble, and re-measure coverage on rn-template + react-navigation-example (confirmed-tier false positives must be 0 per D17d). Do not fetch the partial archive into any sigdb until then.
