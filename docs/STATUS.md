...
- [ ] M5 Pass ladder: one construct fixture per iteration as matcher/writer/checker pass (D12), catalogue row per pass — **6 rungs merged: loop-cond, for-header, expr-rebuild, global-access, call-shape, fn-naming; label-clean (7th) WIP parked; 2 PR-13 bugs queued** (`while`, `do…while`, `for`).
