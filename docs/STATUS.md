...
- **main is RED on 3 `v94 shape:` exact-output assertions** (`19-var-hoisting`, `02-while-loop`, `47-typeof-instanceof-in` in tests/gate/passes/*) — stale since call-shape (f61fdfc) + emitter R2 fix (ba35cdf) improved those functions' emitted shape. **NOT semantic**: T2 equivalence 511/511, 0 DIVERGENT. The running scope-aware ast.ts agent owns those files; if it doesn't green them, a tiny follow-up updates the 3 assertions to the current output. **Design debt (queue an issue): exact-string shape assertions on shared fixtures break on every new rung** — replace with structural checks or per-rung isolated fixtures.
