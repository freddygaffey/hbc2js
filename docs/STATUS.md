...
- **CI fix #3 (next free slot, short Sonnet)**: `tests/gate/decompile/equivalence.test.ts` T2 fails on ubuntu/Node 24 only: `25-generator-delegation.min: trace=INCONCLUSIVE (both traces hit a budget)` — the trace-oracle per-fixture budget used by that test (or by src/harness for the real-decompiler gate) does not scale with `HBC2JS_TIME_SCALE`; make it scale (and consider a per-fixture retry-once on INCONCLUSIVE-by-timeout before failing). Verify with `HBC2JS_TIME_SCALE=0.2 npm test` reproducing locally, then green at 1.
