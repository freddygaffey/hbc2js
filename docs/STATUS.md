...
- **CI**: `review-M4-C1 applyWithGuard vs Hermes VM` must skip INCONCLUSIVE when `tools/hermes-vm/v94` is absent (CI runners); also check `sweep.yml` failures and relax the two Linux timing budgets seen on deb (gate 120 s → scale by core speed or 240 s).
