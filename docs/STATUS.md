...
- [ ] M5 Pass ladder: one construct fixture per iteration as matcher/writer/checker pass (D12), catalogue row per pass — **6 rungs merged: loop-cond, for-header, expr-rebuild, global-access, call-shape, fn-naming; 7 rungs merged incl. label-clean; 2 PR-13 bugs queued (global-access, maxDepth)** (`while`, `do…while`, `for`).
...
- **Gate speedup — ATTEMPT FAILED (2026-08-31), reverted.** A GATE_VERSIONS=[94,99] subset + partial harness parallelism made the gate SLOWER (384s vs ~180s baseline) and was left incomplete/uncommitted; reverted. Re-attempt needs REAL parallelism (worker_thread/subprocess pool sharding the equivalence loop across cores) measured on an UNLOADED machine, as a careful standalone task — not a quick subset tweak. Low priority until budget frees; the ~3min gate is tolerable meanwhile.
