...
- **Review PR #9 (now code, not docs-only)**: head carries T13 (docs/lowering rows 18/19) + the #6 test-guard fix. Reviewer agent on `gh pr diff 9`; verify the T13 codes against a v98/v99 dump of fixtures 23–30; merge on MERGE + green `npm test`. Next free slot.
