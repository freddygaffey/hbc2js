...
## Optional native-side tools (APK / DEX) — never required

`docs/specs/27-native-side.md` §1.2/§1.3. hbc2js reads `classes.dex`,
`AndroidManifest.xml`, `resources.arsc` and the APK container itself with its
**own** read-only TypeScript parsers (`src/native/`), pure Node, no external
binary and no `dependencies` entry. That is the core path and it must keep
working with none of the tools below installed.

| Tool | Licence | What it adds | How it is used |
|---|---|---|---|
| `baksmali` | Apache-2.0 | smali incl. method bodies — the documented way to resolve a value our minimal parser leaves `unresolved` | opt-in oracle on `PATH`, never vendored |
| `apktool` | Apache-2.0 | resources + manifest + smali in one shot | opt-in alternative |
| `jadx` | Apache-2.0 | decompiled Java, nicest to read by hand | opt-in convenience |
| `aapt` / `aapt2` | Apache-2.0 | manifest dump; already used by `src/deps/apk.ts` | opt-in, pre-existing |

Rules: none of these may be added to `dependencies` or vendored into the repo;
a test that wants one must **skip cleanly when it is absent** (the gate never
requires a JVM); and a fact obtained from one of them is recorded with its
source, never merged silently into an own-parser row. The gate's native
fixture is the hermetic synthetic APK generated by `tools/native-fixture/
gen.mjs` (`tests/fixtures/build.sh --native`) — pure Node, no JVM, no
download, and no real-world APK is ever committed (CLAUDE.md / D16 C5).

**One-command walkthrough (spec 27 §L9).** None of the tools above are
needed for the ordinary path: `hbc2js deps app.apk --out ./out` ingests the
APK's native side into `out/native/*.jsonl` with the own-parser reader above,
then immediately reconstructs from those tables — `out/.env`, native
dependencies folded into `out/package.json`, and `out/native-todo/<Module>/`
interface-skeleton stubs for first-party modules — in the same run (see
`docs/DEPS.md`'s `--out` entry). The optional tools in the table only matter
when a hunt needs a value the own-parser reader left `unresolved` (a
`getName()`/`BuildConfig` value living in a method body — see the "Documented
futures" note below).
