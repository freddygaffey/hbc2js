...
| 16 | `arguments` object: real reified object; `arguments[i]`/`.length` reads route through `GetArgumentsPropByVal`/`GetArgumentsLength`'s own lazy-arguments-register operand, not through a shared param slot | `arguments` (42,49) | 94 (O0) | [arguments-object.md](lowering/arguments-object.md) | ✅ single-version | **Reconciled 2026-09-05** (QUEUE item 5, `73-arguments-identity`): the earlier "mapped-arguments aliasing is a live `Store/LoadFromEnvironment` on the *same* slot the parameter uses" wording overstated it and conflicted with `src/runtime/helpers.ts` §8's explicit "Hermes does not alias parameters". Measured directly (real VMs, v84/v96): a parameter write DOES reach a later un-reified `arguments[i]` read in some shapes (`49-arguments-object`'s `namedAliasAfterAssign`: single param, write then immediate read) but Hermes's own compiler proves the write dead and drops it in others (`73-arguments-identity`'s rejected `afterParamWrite` shape: two params, write then a later read) — call-shape-dependent dead-store elimination in hermesc itself, not a stable "aliasing" guarantee the decompiler can rely on or needs to reproduce beyond what the bytecode already encodes. The one case that IS a stable, documented mechanism is `__hbc_argsLive` (`src/runtime/helpers.ts` §8): once `ReifyArguments` has run, `GetArgumentsPropByVal`/`GetArgumentsLength`'s lazy-register operand routes every later read through the *materialised* object, so a write straight into `arguments[i]` (not into a named parameter) is always visible | **Spec 23 written** (`arguments-form`) — it cites readability row `R10`, not this row, because `✅ single-version` fails PL-06; this row stays the lowering provenance
