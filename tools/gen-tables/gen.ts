...
import { parseBuiltinsDef, parseBytecodeListDef, parseTypeofDef } from "./parse-def.ts";
import type { BuiltinDef, BuiltinTable, OpcodeDef, OpcodeTable, OpcodeTableId, OperandTypeName, TypeOfIsTable } from "../../src/tables/types.ts";
...
/**
 * `Typeof.h` exists only from the commit that introduced `TypeOfIs` — the older
 * pins have no such file and get no table (`null`), which is what makes the
 * emitter's refusal correct rather than a guess for v84..v98-2024.
 */
function buildTypeOfIsTable(pin: TablePin): { table: TypeOfIsTable; sha256: string } | null {
  const path = join(ROOT, "third_party", "hermes", pin.id, "Typeof.h");
  if (!existsSync(path)) return null;
  const { text, sha256: hash } = readVendored(pin.id, "Typeof.h");
  return { table: { id: pin.id, hermesCommit: readCommit(pin.id), types: parseTypeofDef(text) }, sha256: hash };
}

function renderTypeOfIsTable(t: TypeOfIsTable, constName: string): string {
  const lines: string[] = [];
  lines.push("// GENERATED by tools/gen-tables/gen.ts — DO NOT EDIT");
  lines.push(`// Source: facebook/hermes @ ${t.hermesCommit} (MIT), include/hermes/FrontEndDefs/Typeof.h`);
  lines.push(`import type { TypeOfIsTable } from "../types.ts";`);
  lines.push(`export const ${constName}: TypeOfIsTable = {`);
  lines.push(`  id: ${JSON.stringify(t.id)},`);
  lines.push(`  hermesCommit: ${JSON.stringify(t.hermesCommit)},`);
  lines.push(`  types: [${t.types.map((n) => JSON.stringify(n)).join(", ")}],`);
  lines.push(`} as const;`);
  lines.push("");
  return lines.join("\n");
}
...
function generateAll(outDir: string): { provenance: string } {
  ...
  for (const pin of PINS) {
    ...
    const typeOfIs = buildTypeOfIsTable(pin);
    if (typeOfIs !== null) {
      writeFileSync(join(outDir, `typeofis-${pin.id}.ts`), renderTypeOfIsTable(typeOfIs.table, constName), "utf8");
      typeofLines.push(`| ${pin.id} | ${typeOfIs.table.hermesCommit} | ${typeOfIs.table.types.join(", ")} | ${typeOfIs.sha256} |`);
    }
  ...
  provenanceLines.push("");
  provenanceLines.push(
    "`Typeof.h`'s `HERMES_TYPEOF_IS_TYPES` macro list, in declaration order — bit `i` " +
      "of the mask is the `i`-th name. There is no negate flag: a `!==` test compiles " +
      "to the complement mask (e.g. 507 = all but `String` = `typeof x !== \"string\"`). " +
      "Pins with no row have no `Typeof.h` at their commit and no `TypeOfIs` opcode, " +
      "so a mask there is `E_EMIT_UNSUPPORTED`, not a guess.",
  );
  provenanceLines.push("");
  provenanceLines.push("| tableId | hermesCommit | bit order | Typeof.h sha256 |");
  provenanceLines.push("|---|---|---|---|");
  provenanceLines.push(...typeofLines);
  provenanceLines.push("");
...
function main(): void {
  ...
  try {
    ...
    const filenames = [
      "PROVENANCE.md",
      ...PINS.flatMap((p) => [`opcodes-${p.id}.ts`, `builtins-${p.id}.ts`]),
      ...PINS.filter((p) => existsSync(join(ROOT, "third_party", "hermes", p.id, "Typeof.h"))).map((p) => `typeofis-${p.id}.ts`),
    ];
    let drift = false;
