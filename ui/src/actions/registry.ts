 ...
 *  and disasm, its context/xrefs, the catalogue, the findings list and the
 *  log tail (so the write shows up in the log pane).
 *  Also drops EVERY cached whole-module file view (`["module-source"]` as a
 *  key PREFIX, `useModuleSource`/`useModuleSources`/`useModuleSourceUpdates`
 *  in `hooks.ts`), not just the written function's own module. Two reasons,
 *  both found by `ui/e2e/code-pane-rename.spec.ts`: a `fn:N` rename changes
 *  every CALLER's rendered text, and callers live in other modules; and the
 *  tree's own row labels come from this same query (`functions[].name` of
 *  each open module), so a rename that did not drop it left the old name in
 *  the tree until a reload. Only MOUNTED module-source queries actually
 *  refetch — the open modules, a handful, never the whole bundle — and the
 *  server answers an untouched module from its own splice cache. */
export function invalidateFn(fn: number | undefined): void {
  ...
  void qc.invalidateQueries({ queryKey: ["module-source"] });
